package com.example.settlement.settle.model.event.handler;

import com.example.settlement.common.event.UnexpectedEvent;
import com.example.settlement.settle.model.domain.BillModel;
import com.example.settlement.settle.model.event.SettleUnSettledInstallmentFee;
import com.example.settlement.settle.model.event.handler.IHandleable;
import jakarta.annotation.Resource;
import lombok.AllArgsConstructor;
import lombok.Setter;
import org.apache.commons.lang3.tuple.Pair;

/**
 *
 * @author yangwu_i
 * @date 2023/5/7 13:21
 */
@Setter
public class SettleProcessInstalmentFee extends AbstractHandler<BillModel> {
    @Resource
    private SettleEventHandler repo;
    @Override
    public UnexpectedEvent handle(BillModel model) {
        if (model.isWaitSettleUnSettledInstallmentFee()) {
            Pair<SettleUnSettledInstallmentFee, UnexpectedEvent> result = model.transferUnSettledInstallmentFee();
            if (result.getRight() != null) {
                return result.getRight();
            }
            SettleUnSettledInstallmentFee event = result.getLeft();
            repo.process(event);
            model.refresh(event);
        }
        return next.handle(model);
    }
}
