...
import com.example.settlement.settle.infra.db.mapper.SettlementBillMapper;
...
public class SettleConfigRepo {
    ...
    public SettleModel getUserSettleModel(Long userId) {
        ...
        if (CollectionUtils.isEmpty(configs)) {
            throw new ErrorNoException(SettlementErrorNo.CONFIG_NOT_FOUND, "配置未找到" + userId);
        }
        ...
        List<SettleBillInfo> infos = settlementBillMapper.selectAllSettleInfo(userId, SettleStatusEnum.unSettled());
        if (infos.stream().anyMatch(e -> !configMap.containsKey(e.configId()))) {
            throw new ErrorNoException(SettlementErrorNo.CONFIG_NOT_FOUND, "配置未找到" + userId);
        }
        Map<SettleId, SettleBillInfo> refer = new HashMap<>();
        // todo?
        infos.forEach(e -> refer.put(new SettleId(e.settleTime(), configMap.get(e.configId()).setting().getUserProduct()), e));
